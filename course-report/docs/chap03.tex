...
本章实验主要回答三个问题：PathTracer 能否稳定识别 \path{agent_pageindex.py} 中全部 22 个控制结构入口；在普通模式、流式模式和列表型消息输入三种场景下，脚本分别会走到哪些路径；三组测试用例的结果并集能否覆盖全部 22 个控制结构入口。
...
\path{agent_pageindex.py} 的总代码量约 130 行，控制结构主要集中在三个函数中。\path{_extract_text} 负责统一处理 \path{None}、字符串、列表等不同类型的消息内容。\path{_print_verbose_stream} 负责消费流式消息，内部包含多层 \texttt{for} 循环、重复消息过滤、工具消息和模型消息分流。\path{main} 负责参数解析、环境变量检查，以及普通模式和 \path{verbose} 模式之间的主流程切换。
...
设计原则如下：被测对象固定为 \path{agent_pageindex.py}，不换成 demo 脚本；外部依赖全部桩化，避免网络和 LLM 输出随机性影响结果；每组用例只改变一种关键消息形态，便于解释覆盖率增量来自哪里；所有结果统一输出为 JSON，方便直接做行号集合分析。
...
每组实验都按相同步骤完成：先在实验进程中安装假的 \path{langchain}、\path{dotenv} 和 \path{pageindex.search} 模块；再根据场景名创建对应的假 Agent；接着用 PathTracer API 将追踪目标固定为 \path{agent_pageindex.py}；然后通过 \path{runpy.run_path} 以 \path{__main__} 方式执行目标脚本；最后用 \path{CoverageReporter} 对执行结果做静态-动态对齐，并输出 JSON 报告。
...
TC2 是增量最大的用例，覆盖率达到 72.7\%。这一组新增的 16 个命中分支由事先设计好的混合流式消息稳定触发。具体来说，这一组消息同时覆盖了 \path{chunk} 不是字典时的过滤、\path{payload} 不是字典时的过滤、重复消息的去重、工具调用消息与工具调用列表遍历、空内容消息的跳过，以及 \path{ToolMessage} 和 \path{AIMessage} 两类消息的分流打印。
...
TC2 是本组实验的主力用例。它把程序显式带进 \path{verbose} 模式，并且在流式返回里依次制造了五类事件：非字典 \path{chunk} 触发第 44 行过滤，字典 \path{chunk} 但 \path{payload} 不是字典触发第 47 行过滤，带 \path{tool_calls} 的消息触发第 57 行和第 58 行，重复消息触发第 52 行，空内容消息、\path{ToolMessage} 和 \path{AIMessage} 分别触发第 64、67、73 行。
...
TC3 是本章实验中用于补齐列表处理路径的关键用例。它依然走普通模式，但把最终消息内容固定成”字符串 + 文本字典”的列表。这样程序在调用 \path{_extract_text} 时，会连续经过第 29 行判断 \path{payload} 是 \path{list}、第 31 行遍历列表元素、第 32 行处理字符串元素、第 34 行处理 \path{\{“type”: “text”\}} 形式的字典元素这四个结构。
...
本章实验表明，PathTracer 可以在 PageIndex 真实入口脚本 \path{agent_pageindex.py} 上稳定工作，并通过受控输入把 22 个控制结构入口全部命中。TC1 覆盖普通模式基线路径，覆盖率为 31.8\%；TC2 覆盖流式处理路径，覆盖率为 72.7\%；TC3 覆盖列表型消息内容路径，覆盖率为 50.0\%；三组结果取并集后，累计覆盖率达到 100.0\%。

这说明 PathTracer 至少在入口脚本这一层，已经具备了较完整的控制结构检测能力。更重要的是，这一覆盖结果来自明确的测试目标与受控输入设计，而不是依赖外部返回内容的偶然形态。

\subsection{仍然存在的局限}

即使本章已经拿到了 100.0\% 的控制结构命中率，也不能把这个结果说得过头。它至少还有两个边界。第一，这个 100.0\% 只代表控制结构入口全覆盖，不代表真假分支方向全覆盖。第 88 行和第 116 行就是最典型的例子：这一行判断被执行到了，但条件真假并没有被分别统计。第二，本章重点覆盖的是 \path{agent_pageindex.py} 入口脚本，而不是 \path{pageindex/search.py} 这类更深层模块。也就是说，当前实验主要验证了”入口脚本路径是否可测”，还没有展开到底层检索逻辑的更深层覆盖分析。
