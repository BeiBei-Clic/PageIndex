...
\end{block}
\vspace{0.05cm}
\begin{block}{\scriptsize while 循环}
{\scriptsize
\begin{verbatim}
...
\end{column}
\begin{column}{0.48\textwidth}
\begin{block}{\scriptsize try/except 异常处理}
{\scriptsize
\begin{verbatim}
...
\end{block}
\vspace{0.05cm}
\begin{block}{\scriptsize 嵌套结构（循环+条件）}
{\scriptsize
\begin{verbatim}
...
\begin{frame}[fragile]
\frametitle{运行命令}
{\footnotesize 一行命令即可追踪任意 Python 程序的执行路径，\textbf{不需要修改被追踪的代码}：}

\vspace{0.1cm}
{\scriptsize
\begin{verbatim}
# 追踪 demo.py，输出文本报告到终端
python -m pathtracer.cli pathtracer/demo.py

# 输出 HTML 格式的覆盖率报告（源码逐行高亮）
python -m pathtracer.cli --format html -o report.html pathtracer/demo.py

# 输出 JSON 格式（方便后续程序处理）
python -m pathtracer.cli --format json -o report.json pathtracer/demo.py

# 静默模式：只输出报告，不显示程序自身的打印
python -m pathtracer.cli --quiet pathtracer/demo.py
\end{verbatim}}

\vspace{0.1cm}
{\footnotesize \textbf{原理：}\texttt{cli.py} 用 \texttt{compile() + exec()} 在追踪环境下运行目标程序，追踪器自动记录一切。}
\end{frame}
...
{\footnotesize 运行 \texttt{demo.py} 后生成的覆盖率报告：}
...
\vspace{0.1cm}
{\footnotesize 80\% 覆盖率意味着有 3 个分支未被执行（如 \texttt{except} 未触发、某个 \texttt{else} 未命中等）。}
\end{frame}

%------------------------------------------------

\begin{frame}
\frametitle{不同测试输入的覆盖对比}
{\footnotesize 以 \texttt{demo\_conditionals} 函数为例，不同输入覆盖不同分支：}
...
\begin{tabular}{lccc}
\toprule
\textbf{测试输入} & \textbf{覆盖分支} & \textbf{未覆盖} & \textbf{覆盖率} \\
\midrule
\texttt{demo\_conditionals(25)} & 4/5 & \texttt{else} (value>50) & 80\% \\
\texttt{demo\_conditionals(-5)} & 1/5 & 4条路径 & 20\% \\
\texttt{demo\_conditionals(0)}  & 2/5 & 3条路径 & 40\% \\
\textbf{综合多次调用} & \textbf{5/5} & \textbf{无} & \textbf{100\%} \\
\bottomrule
\end{tabular}}
\end{table}
...
{\footnotesize
\begin{block}{结论}
单一测试输入只能覆盖部分路径。要达到高覆盖率，需要\textbf{设计多组不同的测试输入}，覆盖所有分支。\\[3pt]
