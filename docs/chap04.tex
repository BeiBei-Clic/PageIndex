...
结合本项目实际情况，推荐按以下方式使用 PathTracer：

\begin{enumerate}
    \item 若只是快速查看某个脚本的覆盖情况，直接使用命令行接口；
    \item 若目标脚本依赖网络、模型返回值或外部工具结果，则优先使用 Python API 封装受控驱动；
    \item 若需要复现本文第三章的实验结果，则直接运行 \path{tests/run_agent_pageindex_experiments.py}。
\end{enumerate}
